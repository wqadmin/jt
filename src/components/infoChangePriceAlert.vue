    ...
    <div class="tf_btn_entrust_alert">
      ...
      <div class="tf_btn_entrust_alert_cont">
        <div>
          ...
          <div>
            <span>价格: </span>
            <p>{{msg.entrust_price}}</p>
          </div>
          <div>
            <span>更改价格: </span>
            <input type="number" v-model="entrustAlertChangePrice">
          </div>
        </div>
...
export default {
  ...
  data() {
    return {
      // entrustOrderChooseList: "", 
      entrustAlertChangePrice: null, // 委托 改价更改价格
    };
  },
  mounted: function() {
    $(function() {
  ...
  methods: {
    tfEntrustAlertCloseFun() { // 委托 改价关闭弹窗
      this.$emit("tfChangeStateFun", false); // 更改显示隐藏状态
    },
    tfEntrustAlertConfirmFun() { // 委托 改价 确认按钮
      let that = this;
      let userToken = that.$store.state.tokenStr; // 获取token
      if (that.entrustAlertChangePrice == null) {
        that.$message.error("请输入更改价格");
      } else {
        that
          .$http({
            url: "/",
            ...
            data: {
              ...
              token: userToken,
              mark: that.msg.id,
              price: that.entrustAlertChangePrice
            }
          })
          .then(res => {
            if (res.data.code == 1) {
