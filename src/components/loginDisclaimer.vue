<template>
  <div class="login_disclaimer_wrap" v-focus tabindex="0">
    <div class="login_disclaimer_wrap_alert">
      ...
      <div class="login_disclaimer_wrap_alert_cont">
        <div v-html="loginDisclaimerCont"></div>
  ...
  mounted: function() {
    ...
    that
      .$http({ //免责协议
        ...
        method: "post",
        data: {
          nozzle: "disclaimer_protocol"
        }
      })
      .then(res => {
        if (res.data.code == 1) {
          that.loginDisclaimerCont = res.data.data.content; // 免责协议条款
